...
static void vADCTask(void *pvParameters) {
    ...
    for ( ;; ) {
        if (xSemaphoreTake(xADCAvgReady, pdMS_TO_TICKS(5000)) == pdTRUE) {
            ledCmd = 2;
            xQueueSend(xLEDQueue, &ledCmd, 0); // turn on blue LED
            ADCAverage = (float)g_ui32ADCSum / 100;
            g_ui32ADCSum = 0;
            g_ui32ADCSampleNum = 0;
            
            /* allocate message buffer - receiver must vPortFree() after using it */
            char *formattedString = pvPortMalloc(MAX_STRING_LEN);
            if (formattedString != NULL) {
                snprintf(formattedString, MAX_STRING_LEN, "Avg ADC Value: %6.2f\n", ADCAverage);
//              ST7735_Message(0, 16, formattedString, strlen(formattedString));
                if (xQueueSend(xUARTQueue, &formattedString, 0) != pdPASS) {
                    /* if queue full/fail, free buffer to avoid leak */
                    vPortFree(formattedString);
                }
            }
            
            vTaskDelay(pdMS_TO_TICKS(500));
            ledCmd = 2;
            xQueueSend(xLEDQueue, &ledCmd, 0); // turn off blue LED
        }
    }
}
